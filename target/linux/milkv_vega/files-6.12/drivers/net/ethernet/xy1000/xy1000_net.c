...
static void __maybe_unused mac_phy_write(struct net_device *dev,unsigned int phy_addr, unsigned int reg_addr,unsigned int data)
{
...
static void __maybe_unused mac_phy_read(struct net_device *dev,unsigned int phy_addr, unsigned int reg_addr,unsigned int *data)
{
	struct mac_eth_priv *priv = netdev_priv(dev);
...
static void sys_ctrl_reset(struct mac_eth_priv *fiber_priv, u32 rst_num, u32 rst_attri)
{
...
static void sys_ctrl_clk(struct mac_eth_priv *fiber_priv, u32 clk_num, u32 clk_attri)
{
 ...
 */
static int fbhm_eth_mac_addr(struct net_device *dev, void *p)
{
	int ret;
...
*/
static int fbhm_mac_eth_open(struct net_device *dev)
{	
...
*/
static int xy1000_net_eth_release(struct net_device *dev)
{			
	struct mac_eth_priv *priv = NULL;
...
*/
static int fbhm_mac_eth_tx(struct sk_buff *skb, struct net_device *dev)
{	
	unsigned long len;
	unsigned char *data;
	unsigned long flags;
...
static void soc_dma_process_done_pkt(unsigned long para)
{
	int ip_len= 0;
...
static int xy1000_net_eth_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	int ret=0;
	//struct mac_eth_priv *priv=netdev_priv(dev);
...
static int fbhm_change_mtu(struct net_device *dev, int new_mtu)
{
...
static int fbhm_probe_vir(struct platform_device *pdev)
{
	...
	priv->dev = &pdev->dev;
	priv->ndev = ndev;
	eth_hw_addr_set(ndev, host_addr_vir);
	ether_setup(ndev);
...
static void fbhm_drv_remove_vir(struct platform_device *pdev)
{
...
//	mac_eth_init(ndev);	
	...
	eth_hw_addr_set(ndev, host_addr);
...
static void fbhm_drv_remove(struct platform_device *pdev)
{
	struct net_device *ndev = platform_get_drvdata(pdev);
